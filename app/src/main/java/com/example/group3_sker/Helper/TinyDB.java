...
public class TinyDB {
    ...
    // Add methods for storing and retrieving cart data based on user ID
    public void putCartData(String userId, ArrayList<Product> cartData) {
        checkForNullKey(userId);
        Gson gson = new Gson();
        String key = "cart_" + userId;
        ArrayList<String> cartDataStrings = new ArrayList<>();
        for (Product product : cartData) {
            cartDataStrings.add(gson.toJson(product));
        }
        putListString(key, cartDataStrings);
    }

    public ArrayList<Product> getCartData(String userId) {
        checkForNullKey(userId);
        String key = "cart_" + userId;
        return getListObject(key);
    }

    public void clearCartData(String userId) {
        checkForNullKey(userId);
        String key = "cart_" + userId;
        remove(key);
    }
